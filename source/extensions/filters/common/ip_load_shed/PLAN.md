...
[DESIGN.md](../../http/ip_load_shed/DESIGN.md) (algorithm + threading),
[ACCOUNTING.md](ACCOUNTING.md) (buffer/CPU resource counting — the hard part),
[experiments](../../../../../experiments/ip_load_shed/README.md) /
[RESULTS.md](../../../../../experiments/ip_load_shed/RESULTS.md) (measured data).
...
4. **Threading**: workers write per-tenant usage into 64 striped shards (21–35ns/op); a
...
5. **Enforcement is split L7/L4 over one shared controller**: the HTTP filter sheds new
   *streams* (503 + `x-envoy-ip-load-shed`, per-stream fairness on multiplexed connections);
...
| piece | where |
| --- | --- |
| solver + shared controller (singleton, pinned; TLS publish; overload/self-contained severity) | `source/extensions/filters/common/ip_load_shed/` |
| shared config proto (`WaterFillConfig`, `tenant_key_source`) | `api/envoy/extensions/filters/common/ip_load_shed/v3/` |
| L7 filter `envoy.filters.http.ip_load_shed` | `source/extensions/filters/http/ip_load_shed/` |
| L4 filter `envoy.filters.network.ip_load_shed` | `source/extensions/filters/network/ip_load_shed/` |
| core: `shed_tenant_load` well-known overload action | `envoy/server/overload/overload_manager.h` |
| tests: solver properties, both factories, HTTP data path | `test/extensions/filters/{common,http,network}/ip_load_shed/` |
| experiment harness + measured data | `experiments/ip_load_shed/` |
...
3. **Productionization** — thread-local double-buffer ledger harvested à la stats-histogram
   merge (drops the striped mutex), weighted water-fill (per-tenant-class weights), LDS-safe

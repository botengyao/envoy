...
Branch: `water_fill_management`. Companion docs: `source/extensions/filters/http/tenant_load_shed/DESIGN.md`
(water-fill algorithm, threading overview), `experiments/tenant_load_shed/` (experiment harness + results).
...
4. **Sampling knob.** The thread-CPU read is a syscall on every platform (~100-500ns), so at
   ...
   as CPU. Measured numbers in `experiments/tenant_load_shed/RESULTS.md`.
...
The accounting hot path is lock-free: each worker owns a thread-local per-tenant *delta*
ledger, and `addUsage()` is a plain map update on the calling thread. (An earlier prototype
used 64 striped `absl::Mutex` shards; its 21-35ns/op measurement is kept in RESULTS.md for
comparison.) The plane copies two proven Envoy patterns verbatim:
...
Tier-1 accounting = one thread-local delta-map add (lock-free; the earlier striped-shard
variant measured 21-35ns/op); shed check = one TLS shared_ptr read + one hash lookup; CPU attribution = two
...
Both filters share one `WaterFillController` (`filters/common/tenant_load_shed`), one usage map,
...
| | L7 `envoy.filters.http.tenant_load_shed` | L4 `envoy.filters.network.tenant_load_shed` |
| --- | --- | --- |
| shed unit | new stream → 503 + `x-envoy-tenant-load-shed` (retriable, per-request granularity, works mid-connection on H2/H3 multiplexed conns) | new connection → close at accept (cheapest possible rejection: no codec, no TLS for plaintext; protects even the accept path) |
...
Harness, configs, and traffic generators live in `experiments/tenant_load_shed/`; measured results

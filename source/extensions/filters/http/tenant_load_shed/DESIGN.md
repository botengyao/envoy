# Tenant-based water-fill load shedding for multi-tenant ambient Envoy
...
Each worker owns a thread-local per-tenant *delta* ledger (`absl::flat_hash_map`), so the
accounting write path is a plain map update with no lock and no atomics. Every evaluation
interval the controller posts a harvest closure to each worker (which moves the ledger into a
shared accumulator on the worker's own thread) and, in the completion callback on the main
thread, folds the deltas into the authoritative per-tenant usage map — the same choreography
as `ThreadLocalStoreImpl` histogram merging.
...
3. **Optional decay.** An EWMA or windowed decay on `u_i` so idle-but-open streams gradually
   stop counting.
...
3. **Publish**: build an immutable `ShedSnapshot{severity, water_level, shed_tenants}` and
...
`TenantLoadShedFilter::decodeHeaders`:
...
3. If shed: `sendLocalReply(503, "tenant over water level")` with
   `x-envoy-tenant-load-shed: true` (503 is retriable-after-backoff by well-behaved clients;
   configurable to 429).
...
```yaml
http_filters:
- name: envoy.filters.http.tenant_load_shed
  typed_config:
    "@type": type.googleapis.com/envoy.extensions.filters.http.tenant_load_shed.v3.TenantLoadShed
    water_fill:
...
| stat | type | meaning |
| --- | --- | --- |
| `tenant_load_shed.shed_total` | counter | streams rejected by water-fill |
| `tenant_load_shed.severity` | gauge (‰) | current shed severity s × 1000 |
| `tenant_load_shed.water_level_bytes` | gauge | current water level L |
| `tenant_load_shed.tenants_tracked` | gauge | tenants with non-zero usage |
| `tenant_load_shed.tenants_shed` | gauge | tenants above the water line |
| `tenant_load_shed.total_usage_bytes` | gauge | Σ u_i |

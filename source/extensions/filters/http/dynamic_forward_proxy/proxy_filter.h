...
private:
  class HostCandidateLookup
      : public Extensions::Common::DynamicForwardProxy::DnsCache::LoadDnsCacheEntryCallbacks {
  public:
    HostCandidateLookup(ProxyFilter& parent, uint32_t candidates)
        : parent_(parent), candidates_(candidates) {}

    // Extensions::Common::DynamicForwardProxy::DnsCache::LoadDnsCacheEntryCallbacks
    void onLoadDnsCacheComplete(
        const Extensions::Common::DynamicForwardProxy::DnsHostInfoSharedPtr& host_info) override {
      parent_.onHostCandidateLoaded(candidates_, host_info);
    }

    ProxyFilter& parent_;
    // Number of candidates that name this host.
    const uint32_t candidates_;
  ...
  uint32_t usable_host_candidates_{};
  bool proxying_host_candidates_{};

# ip_load_shed experiments

Reproducible experiments for the water-fill load-shedding filters (L7 + L4) and the
accounting design (`source/extensions/filters/common/ip_load_shed/ACCOUNTING.md`).
Results: `RESULTS.md`.
...
`[0,1]` to `/tmp/ip_load_shed_exp/pressure` (the `injected_resource` overload monitor), so no
...
`ip_load_shed.*` admin stats land in `results_<mode>.json`.
...
clang++ -O2 -std=c++17 bench_accounting.cc \
    ../../source/extensions/filters/common/ip_load_shed/water_fill.cc -I../../ \

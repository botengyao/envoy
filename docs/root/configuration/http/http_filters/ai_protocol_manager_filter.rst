...
Two filter state keys describe the two sides of a stream. Place the filter that writes either key
before this filter; :ref:`AI filters <config_http_filters_ai_protocol_manager_ai_filters>` read
both.

``envoy.ai.downstream_api``
  How the client speaks to the gateway: a :ref:`DownstreamApi
  <envoy_v3_api_msg_type.ai.v3.DownstreamApi>` as JSON, or an :ref:`LLMProtocol
  <envoy_v3_api_enum_type.ai.v3.LLMProtocol>` value name such as ``ANTHROPIC_MESSAGES``. Its
  protocol takes precedence over the route's request declaration and makes the request a declared
  AI endpoint. When the key is absent, the filter in the downstream filter chain writes the route's
  declared protocol there, so an upstream instance parses the request the same way. An object
  without a protocol defers to the route and is not replaced.

``envoy.ai.upstream_target``
  The :ref:`UpstreamTarget <envoy_v3_api_msg_type.ai.v3.UpstreamTarget>` as JSON, read only in a
  cluster's upstream filter chain. Token usage is extracted from the response in its protocol.

The upstream target decides where the request goes and how it authenticates, so only
trusted, configuration-driven writers may set it, never from request content; with the
:ref:`set_filter_state filter <config_http_filters_set_filter_state>`, use a constant value. The
downstream API only decides how the client's own request is read, so it may be derived from the
request, for example from a header.

Both objects serialize as JSON. ``llm_protocol`` is a field of both, and ``authority``, ``model``
and ``preset`` are fields of the upstream target, so one constant target can also pick the host of
a :ref:`dynamic forward proxy <config_http_filters_dynamic_forward_proxy>`:
  ...
  typed_per_filter_config:
    envoy.filters.http.set_filter_state:
      ...
      - object_key: envoy.ai.upstream_target
        format_string:
          text_format_source:
            inline_string: '{"llm_protocol": "OPENAI_CHAT_COMPLETIONS", "authority": "api.openai.com"}'
...
set, and ``host_rewrite`` sets ``Host`` to the same authority.

...
which realigns the buffer at the cost of one short TLS record. Guarded by runtime flag
``envoy.reloadable_features.tls_avoid_repeated_linearize`` (default ``true``).

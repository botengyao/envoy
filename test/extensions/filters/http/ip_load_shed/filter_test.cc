...
using Filters::Common::IpLoadShed::WaterFillController;

class IpLoadShedFilterTest : public testing::Test {
protected:
  IpLoadShedFilterTest() {
  ...
  std::unique_ptr<IpLoadShedFilter>
  makeFilter(NiceMock<Http::MockStreamDecoderFilterCallbacks>& callbacks, const std::string& ip) {
    ...
    auto filter = std::make_unique<IpLoadShedFilter>(filter_config_);
    filter->setDecoderFilterCallbacks(callbacks);
    return filter;
  }

  uint64_t totalUsage() {
    return TestUtility::findGauge(context_.store_, "ip_load_shed.total_usage_bytes")->value();
  }
...
TEST_F(IpLoadShedFilterTest, RejectAllAtSaturation) {
  ...
  EXPECT_CALL(callbacks, sendLocalReply(Http::Code::ServiceUnavailable, _, _, _, "ip_load_shed"));
...
// destruction.
TEST_F(IpLoadShedFilterTest, AdmitAccountAndReleaseLifecycle) {
  NiceMock<Http::MockStreamDecoderFilterCallbacks> callbacks;
  auto filter = makeFilter(callbacks, "10.0.0.1");
...
TEST_F(IpLoadShedFilterTest, WaterFillShedsHeavyTenantOnly) {
  ...
  EXPECT_CALL(heavy_retry_callbacks,
              sendLocalReply(Http::Code::ServiceUnavailable, _, _, _, "ip_load_shed"));
  ...
  EXPECT_EQ(1, TestUtility::findGauge(context_.store_, "ip_load_shed.tenants_shed")->value());
}

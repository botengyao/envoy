...
public:
  ...
  // zero inline-string threshold leaves the field unset, so the default applies.
  void createFilter(bool parse_unconfigured_routes = false, bool publish_request_info = false,
    ...
    if (max_unconfigured_body_bytes.has_value()) {
      proto.mutable_request_handling()
          ...
          ->set_value(max_unconfigured_body_bytes.value());
    }
  ...
  // Run all posted and scheduled callbacks, including work enqueued while
  // draining. Strict requests now pass through FilterManager serialization,
  // which may chain several BufferManager replay ranges.
  void drain() {
    while (true) {
      while (!posted_.empty()) {
      ...
      if (replay_cb_ == nullptr || !replay_cb_->enabled()) {
        break;
      }
...
// The body is offloaded chunk-by-chunk and serialized for replay once end_stream
// is seen, with end_stream propagated on the final injected frame.
...
// A declared endpoint's payload is parsed as it is offloaded and serialized back
// into equivalent JSON for downstream filters.
TEST_F(AiProtocolManagerFilterTest, ParsesDeclaredEndpointPayloadAndReplaysEquivalentJson) {
  setRouteConfig();
  EXPECT_EQ(decodeHeadersEngaging(), Http::FilterHeadersStatus::StopIteration);
...
TEST_F(AiProtocolManagerFilterTest, RaisedInlineStringThresholdKeepsLargeValuesInline) {
  createFilter(/*parse_unconfigured_routes=*/false, /*publish_request_info=*/false,
               /*max_unconfigured_body_bytes=*/std::nullopt,
               /*default_protocol=*/envoy::type::ai::v3::API_PROTOCOL_UNSPECIFIED,
               /*inline_string_threshold_bytes=*/4096);
  setRouteConfig();
  EXPECT_EQ(decodeHeadersEngaging(), Http::FilterHeadersStatus::StopIteration);
...
// Unknown fields are permitted and preserved semantically.
...
// declared API's schema, serialized, and forwarded.

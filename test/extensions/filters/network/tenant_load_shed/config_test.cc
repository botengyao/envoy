...
TEST(TenantLoadShedNetworkConfigTest, SelfContainedModeCreatesFilter) {
  ...
  envoy::extensions::filters::network::tenant_load_shed::v3::TenantLoadShed proto_config;
  proto_config.mutable_water_fill()->set_max_heap_size_bytes(2ULL * 1024 * 1024 * 1024);

  TenantLoadShedConfigFactory factory;
...
TEST(TenantLoadShedNetworkConfigTest, MissingPressureSourceRejected) {
  NiceMock<Server::Configuration::MockFactoryContext> context;
  envoy::extensions::filters::network::tenant_load_shed::v3::TenantLoadShed proto_config;
